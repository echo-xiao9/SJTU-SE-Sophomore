// This mr_coordinator is responsible for scheduling distributed tasks. 
// When the worker tells him that the task is complete, he submits it.
// meanwhile it use RPC 
#include <string>
#include <vector>
#include <stdio.h>
#include <stdlib.h>
...
{
  // Lab2 : Your code goes here.
  cout << "Submit task, taskType: " << taskType << " index: " << index << endl;
  this->mtx.lock();
  if ((mr_tasktype)taskType == MAP)
  {
    mapTasks[index].isCompleted = true;
    mapTasks[index].isAssigned = false;
    this->completedMapCount++;
  }
  else if ((mr_tasktype)taskType == REDUCE)
  {

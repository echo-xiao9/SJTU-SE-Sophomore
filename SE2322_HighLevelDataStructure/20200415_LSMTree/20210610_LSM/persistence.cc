...
//            if(i%100==0)cout<<i<<' ';
            ...
            case 1:
                    if(std::string(i+1, 's')!=store.get(i)){
                        cout<<"case1: "<<i<<' ' <<store.get(i).length()<<endl;
                    }
                store.put(i, std::string(i+1, 't'));
                break;
            case 2:
                    if( store.get(i)!="")cout<<"case2:"<<i<<' '<<store.get(i).length()<<endl;
                break;
            case 3:
                if(std::string(i+1, 's')!= store.get(i))
                    cout<<"case3:"<<i<<" "<<store.get(i).length()<<endl;
                break;
            default:
                assert(0);
            }
